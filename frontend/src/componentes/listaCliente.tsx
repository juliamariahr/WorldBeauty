  ...
  const updateCliente = async (cliente: Cliente) => {
    try {
      const response = await axios.put<Cliente>(
        `http://localhost:32832/clientes/${cliente.id}`,
        cliente
      );
      setClientes(
        clientes.map((c) => (c.id === cliente.id ? response.data : c))
      );
      ...
      <div id="modal1" className="modal" ref={modalRef}>
          ...
          {selectedCliente && (
            <>
              ...
              <input
                id="sobreNome"
                type="text"
                value={selectedCliente.sobreNome}
                onChange={handleInputChange}
              />
              <input
                id="email"
              ...
              <input
                id="rua"
                type="text"
                value={selectedCliente.endereco.rua}
                onChange={handleInputChange}
              />
              <input
                id="numero"
              ...
              <input
                id="codigoPostal"
                type="text"
                value={selectedCliente.endereco.codigoPostal}
                onChange={handleInputChange}
              />
            </>
          )}
        ...
        <div className="modal-footer">
          <button
            ...
            onClick={() => updateCliente(selectedCliente!)}
          >
            Salvar
          </button>
